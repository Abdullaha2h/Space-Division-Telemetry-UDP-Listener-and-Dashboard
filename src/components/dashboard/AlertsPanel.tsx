    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-semibold text-white/50 uppercase tracking-widest">Active Alerts</span>
                <Badge className="ml-auto text-[10px] h-5 bg-white/10 text-white/60 border-white/10">{alerts.length}</Badge>
            </div>
            ...
            <div className="flex-1 min-h-0 overflow-hidden rounded-lg border border-white/10">
                <ScrollArea className="h-full">
                    <div className="p-1.5 space-y-1.5">
                        {alerts.length === 0 ? (
                            <p className="text-[11px] text-white/30 text-center py-6">No active alerts</p>
                        ) : alerts.map((alert) => (
                            <div
                                key={alert._id}
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-white/30 text-[10px]">{new Date(alert.timestamp).toLocaleTimeString()}</span>
                                    </div>
                                    <p className="text-white/50 truncate">{alert.message}</p>
                                </div>
                            </div>
                        ))}
                    </div>
