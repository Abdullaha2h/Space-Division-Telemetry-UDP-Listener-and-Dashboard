...
export default function HistoryModal({ onClose }: Props) {
    ...
    const dateInputClass = "h-7 rounded-md border border-white/10 bg-white/5 px-2 text-xs text-white/70 focus:outline-none focus:border-emerald-500/40 transition-colors";
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="w-full max-w-2xl mx-4 rounded-2xl border border-white/10 bg-black/60 backdrop-blur-2xl shadow-2xl">
                ...
                <div className="flex items-center justify-between px-5 py-3 border-b border-white/10">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <Button variant="ghost" size="sm"
                            ...
                            disabled={loading}
                            className="h-7 px-2 text-[10px] gap-1 hover:bg-white/10 text-white/40 hover:text-white/70"
                        ...
                        {/* Filter toggle */}
                        <Button variant="ghost" size="sm"
                            className={`h-7 w-7 p-0 hover:bg-white/10 ${showFilters ? "text-emerald-400" : "text-white/30"}`}
                            onClick={() => setShowFilters((v) => !v)} title="Toggle date filters">
                            <Filter className="w-3.5 h-3.5" />
                        </Button>
                        {/* Manual refresh */}
                        <Button variant="ghost" size="sm"
                            className="h-7 w-7 p-0 hover:bg-white/10 text-white/30"
                ...
                {showFilters && (
                    <div className="flex flex-wrap items-end gap-3 px-5 py-3 border-b border-white/10 bg-white/[0.03]">
                        ...
                        <Button variant="ghost" size="sm" onClick={handleReset}
                            className="h-7 text-xs text-white/25 hover:text-white/50 hover:bg-white/5">
                            Reset
                        </Button>
                    </div>
                )}
                    ...
                    {loading && allRecords.length === 0 ? (
                        ...
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 z-10 bg-black/80 backdrop-blur-sm">
                                <tr className="border-b border-white/10 text-[10px] text-white/25 uppercase">
                            ...
                            <tbody>
                                {pageRecords.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="text-center py-8 text-white/25 text-xs">
                                            No records found
                                        </td>
                                    </tr>
                                ) : pageRecords.map((r) => {
                                    ...
                                    return (
                                        <tr key={r._id} className="border-b border-white/[0.06] hover:bg-white/5 transition-colors">
                                            <td className="px-5 py-2 text-xs text-white/35">{new Date(r.timestamp).toLocaleString()}</td>
                                            <td className={`px-4 py-2 text-xs font-medium ${r.battery_voltage < 12000 ? "text-yellow-400" : "text-white/65"}`}>
                                                {r.battery_voltage}
                                            </td>
                                            <td className={`px-4 py-2 text-xs font-medium ${r.msi_temperature > 40 ? "text-red-400" : "text-white/65"}`}>
                                                {r.msi_temperature}
                                            </td>
                ...
                <div className="flex items-center justify-between px-5 py-2.5 border-t border-white/10 text-xs text-white/25">
                    <span>{allRecords.length} total · page {page} of {totalPages}</span>
                    <div className="flex items-center gap-1">
                        {loading && <Loader2 className="w-3 h-3 animate-spin mr-1" />}
                        <Button variant="ghost" size="sm"
                            className="h-6 w-6 p-0 hover:bg-white/10 text-white/35"
